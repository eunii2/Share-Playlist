...
public interface FriendListRepository extends JpaRepository<FriendList, Integer>{
        @Query("SELECT fl.id, fl.id2.userid, fl.id2.username FROM FriendList fl WHERE fl.id1.userid=:id1")
        public abstract List<List<String>> findId2ById1Userid(String id1);

        @Query("SELECT fl FROM FriendList fl WHERE fl.id1.userid=:id1 AND fl.id2.userid=:id2")
